...
export const signup = async (
  ...
  try {
    ...
    const verificationUrl = `${req.protocol}://${req.get(
      "host"
    )}/api/v1/auth/verify-token?token=${verificationToken}`;
...
export const verifiedToken = async (
  ...
  try {
    ...
    res
      .status(500)
      .json({ errMessage: "An error occurred during verification", error });
  }
};
...
export const signin = async (
  ...
  try {
    ...
    if (!user) {
      return res
        .status(401)
        .json({ errMessage: "User not found. Please register." });
    }
    ...
    if (!user.isVerified) {
      return res.status(401).json({
        errMessage: "User is not verified. Please verify your email.",
      });
    }
